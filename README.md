...
- 阶段①A先把固定 `smpl22-mixamo-v1` 骨架拟合并嵌入原始角色；除躯干中轴外，还会从 T/A Pose 网格横截面估计上臂、肘、腕和双脚中心，并通过中央头部前表面估计头颅旋转中心、由肩线约束颈部，使关节进入模型体积中心。拟合数值写入 `character_skeleton_fit.json`。
