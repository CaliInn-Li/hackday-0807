...
def as_rotation_matrix(value, device):
...
def normalized_rotations(params, device):
    root = as_rotation_matrix(params["global_orient"], device)
    body = as_rotation_matrix(params["body_pose"], device)
    if root.ndim == 3:
        root = root[:, None]
    if body.ndim == 3:
...
def main():
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("GVHMR motion extraction requires CUDA, but torch.cuda.is_available() is false")
    ...
    rotations = normalized_rotations(params, device).cpu().numpy().astype(np.float32)
    translations = torch.as_tensor(params["transl"], device=device).cpu().numpy().astype(np.float32)
