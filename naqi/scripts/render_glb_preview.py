...
def main():
    ...
    for name, energy, offset in (
        ("PreviewKey", 80, Vector((radius * 2, -radius * 3, radius * 3))),
        ("PreviewFill", 35, Vector((-radius * 2, -radius, radius))),
        ("PreviewRim", 55, Vector((radius, radius * 2, radius * 2))),
    ):
    ...
    def follow_root(scene_arg):
        index = max(0, min(scene_arg.frame_current - scene_arg.frame_start, len(root_positions) - 1))
    ...
    bpy.app.handlers.frame_change_pre.append(follow_root)
    scene.frame_set(scene.frame_start)
    bpy.ops.render.render(animation=True)
    bpy.app.handlers.frame_change_pre.remove(follow_root)
    print({"input": str(input_path), "output": str(output_path), "frames": scene.frame_end, "fps": scene.render.fps, "cycles_device": cycles_device})
