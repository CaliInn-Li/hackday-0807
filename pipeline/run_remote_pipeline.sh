...
for path in "$VIDEO" "$CHARACTER" "$SKIN_PY" "$GVHMR_PY" "$BLENDER_BIN"; do
...
"$BLENDER_BIN" --background --python "$SCRIPT_DIR/prepare_and_test_rig.py" -- \
  --input "$RIGGED" \
  --mapping "$SCRIPT_DIR/skintokens_mixamo_mapping.json" \
  --clean-output "$CLEAN" \
...
echo "GVHMR comparison video: $GVHMR_ROOT/$VIDEO_STEM"
